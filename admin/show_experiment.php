...
$id = (int)$_GET['id'];
$exp = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix."WPTestMonkey_experiments WHERE id=%d", $id));
...
$variations = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->prefix."WPTestMonkey_variations WHERE experiment_id=%d", $id));
$goals = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->prefix."WPTestMonkey_goals WHERE experiment_id=%d", $id));
?>
...
<div class="wrap">
  ...
  <p>
    ...
    <a href="?page=WPTestMonkey&amp;action=edit_experiment&amp;id=<?php echo $exp->id ?>">Edit experiment</a>
    <small>|</small>
    <small><a href="?page=WPTestMonkey&amp;action=delete_experiment&amp;id=<?php echo $exp->id ?>" style="color: #c00;">Delete experiment</a></small>
  </p>
  <?php
  $best_rate = 0;
  ...
  <?php if ($exp->type == 'content') { ?>
    <p>
      In <strong>posts, pages, or widgets</strong>, insert this code: <code>[WPTestMonkey experiment="<?php echo $id ?>"]</code><br />
      Or, as <strong>PHP code</strong> in themes etc.: <code>&lt;?php WPTestMonkey_experiment(<?php echo $id ?>) ?&gt;</code> or <code>&lt;?php echo WPTestMonkey_get_experiment(<?php echo $id ?>) ?&gt;</code>
    </p>
  ...
  <p>
    ...
    <code>[WPTestMonkey experiment="<?php echo $id ?>" variable="name"]</code><br />
    Or, as <strong>PHP code</strong> in themes etc.: <code>&lt;?php WPTestMonkey_name(<?php echo $id ?>) ?&gt;</code> or <code>&lt;?php echo WPTestMonkey_get_name(<?php echo $id ?>) ?&gt;</code>
  </p>
  <p>
  ...
  <table class="wp-list-table widefat" cellspacing="0">
      ...
      foreach ($variation_stats as $var) {
        ...
        ?>
      	<tr>
      	  <td>
            <a href="?page=WPTestMonkey&amp;action=edit_variation&amp;id=<?php echo $var->id ?>"><strong><?php echo $var->name ?></strong></a>
            <?php if (!$var->active) { ?>
              <small>(disabled)</small>
            <?php } ?>
            <br />
            <small>
              <a href="?page=WPTestMonkey&amp;action=edit_variation&amp;id=<?php echo $var->id ?>">Edit</a>
              |
              <?php if ($var->active) { ?>
                <a href="?page=WPTestMonkey&amp;action=set_variation_active&amp;active=0&amp;id=<?php echo $var->id ?>">Disable</a>
              <?php } else { ?>
                <a href="?page=WPTestMonkey&amp;action=set_variation_active&amp;active=1&amp;id=<?php echo $var->id ?>">Enable</a>
              <?php } ?>
              |
              <a href="?page=WPTestMonkey&amp;action=delete_variation&amp;id=<?php echo $var->id ?>">Delete</a>
            </small>
      	  </td>
  ...
  <p>
    <input type="button" value="Add new variation" class="button-secondary" onclick="document.location = '?page=WPTestMonkey&amp;action=add_variation&amp;experiment_id=<?php echo $id ?>';" />
  </p>
  ...
  <table class="wp-list-table widefat" cellspacing="0">
      ...
      <?php foreach ($goals as $goal) { ?>
        <tr>
          <td>
            <a href="?page=WPTestMonkey&amp;action=edit_goal&amp;id=<?php echo $goal->id ?>"><strong><?php echo $goal->name ?></strong></a><br />
            <small>
              <a href="?page=WPTestMonkey&amp;action=get_tracking_code&amp;id=<?php echo $goal->id ?>">Get tracking code</a>
              |
              <a href="?page=WPTestMonkey&amp;action=edit_goal&amp;id=<?php echo $goal->id ?>">Edit</a>
              |
              <a href="?page=WPTestMonkey&amp;action=delete_goal&amp;id=<?php echo $goal->id ?>">Delete</a>
            </small>
          </td>
        </tr>
      <?php } ?>
  ...
  <p>
    <input type="button" value="Add new goal" class="button-secondary" onclick="document.location = '?page=WPTestMonkey&amp;action=add_goal&amp;experiment_id=<?php echo $id ?>';" />
  </p>
  
  <p>
    &laquo; <a href="?page=WPTestMonkey">Back to experiments</a>
  </p>
